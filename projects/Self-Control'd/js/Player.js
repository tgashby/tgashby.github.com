...
Player.prototype.moveLeft = function() {
    ...
    if (this.vel.x < -1 * this.velmax)
        this.vel.x = -1 * this.velmax;
}
...
Player.prototype.update = function() {
    ...
    this.haunted = this.nearbyEnemies();
    this.canJump = !this.haunted; // If you're scared, you can't jump.

    if (!Key.isDown(Key.LEFT) && !Key.isDown(Key.RIGHT) && !this.haunted) {
        this.vel.x = 0;
        if (this.sprite.currentAnim != "idle") {
            this.sprite.setAnimation("idle");
        }
    } else if (!this.haunted) {
